...
const filteredUsers = computed(() => {
  ...
  const exact = users.value.find(f => f.name.toLowerCase() === search)
  if (exact) return [exact]

  const partial = users.value.find(f => f.name.toLowerCase().includes(search))
  return partial ? [partial] : []
})
...
<template>
  ...
  <div class="files panel" v-if="currentMenu === 'files'">
  ...
  <div class="users panel" v-if="currentMenu === 'users'">
    ...
    <div class="grid" v-for="item in users">
      ...
      <div class="btn">
        <button @click="openMessenger()"><i class="ti ti-message"></i></button>
        <button @click="showUserInfo()"><i class="ti ti-id"></i></button>
      </div>
    </div>
  </div>

  <footer>
    <span>SunnyCDN v0.4</span>
